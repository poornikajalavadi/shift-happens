...
Downloads current production model metadata from GCS
Compares new model ROC-AUC against production ROC-AUC
If new model AUC > production AUC → push approved, old model archived
If new model AUC ≤ production AUC → push blocked, existing model stays
